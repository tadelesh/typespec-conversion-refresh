...
- **ManualReview**
  There are some changelog items that need manual review. Please check the PR and update the status after review.
...
If no "tspconfig.yaml" is found, add "NoSpec" to "tspconfig" column, and skip all the following steps for this row.
...
If there is a `tsplocation.yaml` file under the `SdkFolder`, it means the Go SDK has already been released from TypeSpec. Add "AlreadyTypeSpec" to "Go" column, and skip all the following steps for this row.

3. Find the first conversion API version for the service, and add it to "SpecApiVersion" column
...
```
4. Find SDK API version and update the "SdkApiVersion" column
...
5. Compare `SpecApiVersion` with `SdkApiVersion`
...
6. Generate SDK {sdk} via pipeline

If "SdkPr" column has a PR link, it means we have already generated the SDK. We can skip to next step.

Else, run the pipeline https://dev.azure.com/azure-sdk/internal/_build?definitionId=7421 via REST API
