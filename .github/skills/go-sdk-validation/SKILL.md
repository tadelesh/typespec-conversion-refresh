...
5. Go to the sdk repo root folder, if branch `{service}-{armservice}-swagger-baseline` not exists, regenerate sdk by command `generator release-v2 {sdk_repo_path} {specs_repo_path} {service} {armservice} --skip-generate-example` and rename the generated branch to `{service}-{armservice}-swagger-baseline`(if branch exists, just commit changes to it) and push it to remote,this will be the branch used for step 8 to compare the changelog and classify breaking changes. `{service}` and `{armservice}` can be extracted from the SDK folder path `sdk/resourcemanager/{service}/{armservice}`.
