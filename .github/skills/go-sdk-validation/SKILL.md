...
Find the first item in the `Version` enum. The enum is specified in the `@versioned` decorator, usually in `main.tsp`:
...
### 6. Compare API versions

Compare the TypeSpec API version (step 4) with the SDK API version (step 5).

If they are the same, skip to step 8.

If they are different, proceed to step 7 to generate a swagger baseline.

### 7. Generate SDK with Swagger for version-not-equal services

This step generates the SDK from the pre-conversion swagger spec to establish a proper changelog baseline.

1) Go to the folder of `tspconfig.yaml` in specs repo, find the commit this config is first created. Go to the `specification/{SpecFolder}` in specs repo, get the -1 commit ID of that commit (the "SwaggerCommit").
...
3) Go to the SDK folder in the sdk repo, edit the `autorest.md` file: add or update the tag in the yaml to `tag: {SwaggerTag}`. Also update the `require` URLs in `autorest.md` to point to the SwaggerCommit and the correct readme.md path found in step 2). The readme.md may be in a nested subfolder (e.g., `resource-manager/Microsoft.X/SubFolder/readme.md`) rather than at the `resource-manager/` root.
...
Extract the latest version's changelog from the `CHANGELOG.md` file in the SDK folder generated by TypeSpec in step 5.

**If versions are equal (step 7 was skipped):** review all changelog items from the TypeSpec-generated changelog.

**If versions are not equal (step 7 was executed):** also extract the latest version's changelog from the swagger-generated `CHANGELOG.md` in step 7. Compare the two changelogs and compute the **diff** — only the items that appear in the TypeSpec changelog but NOT in the swagger changelog need to be reviewed. Items present in both changelogs are caused by the API version difference, not the TypeSpec conversion, and can be ignored.
...
After making fixes:
1) Commit and push the changes in the specs repo.
...
Once all resolvable breaking changes have been addressed:

**If working from a conversion PR:** leave a comment on the PR summarizing:
- The Go SDK generation result
- The remaining acceptable breaking changes (if any)
- The fixes applied

**If working from a tspconfig path:** report the summary to the user.
